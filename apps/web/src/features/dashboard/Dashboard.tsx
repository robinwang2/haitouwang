...
export function Dashboard({
  ...
  return (
    <main className={styles.shell}>
      {effectivePaused && (
        <aside
          className={`${styles.globalBand} ${safety === 'unknown' ? styles.unknownBand : ''}`}
          role="status"
        >
      ...
      <header className={styles.hero}>
        ...
        <div className={styles.heroActions}>
          {visibleTodos.length > 0 && (
            <a className={styles.primaryButton} href={`#todo-${visibleTodos[0].id}`}>
              {copy.nextTask}
            </a>
          )}
          <a className={styles.secondaryButton} href="/jobs?import=true">
            {copy.importJob}
          </a>
        </div>
      </header>
      ...
      <section aria-labelledby="safety-heading" className={styles.safetyCard}>
        <div className={styles.safetyCopy}>
          ...
          <p
            className={`${styles.statusLine} ${safety === 'online' ? styles.online : styles.offline}`}
          >
            ...
            {agent.last_seen_at && (
              <>
                {' '}
                · {copy.heartbeat} {formatRelative(agent.last_seen_at, locale)}
              </>
            )}
          </p>
        </div>
        <div className={styles.safetyMetric}>
          <span>{copy.affected}</span>
          <strong>{affectedTaskCount(tasks)}</strong>
        </div>
        <div className={styles.switches}>
          <div className={styles.switchRow}>
            <div>
              <strong>{copy.safetyTitle}</strong>
              <small>{effectivePaused ? copy.paused : copy.assisted}</small>
            </div>
            <button
          ...
          <div className={styles.switchRow}>
            <div>
              <strong>{copy.automaticSubmit}</strong>
              <small>{copy.permanentlyOff}</small>
            </div>
            <button
              aria-checked={AUTOMATIC_SUBMISSION_ENABLED}
              aria-label={`${copy.automaticSubmit}: ${copy.permanentlyOff}`}
              className={styles.switch}
              disabled
              role="switch"
              type="button"
      ...
      <div className={styles.mainGrid}>
        <section aria-labelledby="todo-heading" className={styles.todoSection}>
          <div className={styles.sectionHeader}>
            <div>
              <p className={styles.sectionEyebrow}>{copy.needsAction}</p>
              <h2 id="todo-heading">{copy.todoHeading}</h2>
              <p>{copy.todoDescription}</p>
            </div>
          </div>
          {visibleTodos.length > 0 ? (
            <ol className={styles.todoList}>
              {visibleTodos.map((todo, index) => (
                <TodoCard copy={copy} index={index + 1} key={todo.id} locale={locale} todo={todo} />
              ))}
            </ol>
          ) : (
        ...
        <aside className={styles.sidebar}>
          <section aria-labelledby="today-heading" className={styles.summaryCard}>
            <div className={styles.sectionHeader}>
              <div>
                <p className={styles.sectionEyebrow}>{copy.dayRange}</p>
                <h2 id="today-heading">{copy.todayHeading}</h2>
              </div>
            </div>
            <dl className={styles.statsGrid}>
              <Stat
                href="/jobs?created=today"
                label={copy.discovered}
                value={pageState === 'empty' ? 0 : 12}
              />
              <Stat
                href="/jobs?match=high"
                label={copy.highMatch}
                value={pageState === 'empty' ? 0 : 5}
              />
              <Stat
                href="/jobs?decision=blocked"
                label={copy.blocked}
                value={pageState === 'empty' ? 0 : 3}
              />
              <Stat
                href="/applications?status=submitted"
                label={copy.submitted}
                value={pageState === 'empty' ? 0 : verifiedSubmissions}
              />
            </dl>
          </section>
          ...
          <section aria-labelledby="activity-heading" className={styles.activityCard}>
            <div className={styles.sectionHeader}>
              <div>
                <p className={styles.sectionEyebrow}>{copy.auditTrail}</p>
                <h2 id="activity-heading">{copy.activityHeading}</h2>
              </div>
              ...
              {visibleApplications.slice(0, 4).map((application) => (
                <li key={application.id}>
                  ...
                  <div>
                    ...
                    <p>
                      {copy.applicationLabel} {application.id.slice(-4)}
                    </p>
                    <time dateTime={application.updated_at}>
                      {formatRelative(application.updated_at, locale)}
                    </time>
                  </div>
                </li>
              ))}
...
function TodoCard({
  ...
  return (
    <li className={`${styles.todoCard} ${styles[todo.kind]}`} id={`todo-${todo.id}`}>
      <span className={styles.todoIndex} aria-hidden="true">
      ...
      <a aria-label={`${copy.open}: ${todo.title}`} href={`/applications/${todo.id}`}>
        {copy.open}
        <span aria-hidden="true"> →</span>
      </a>
    </li>
  ...
  return (
    <div className={styles.dialogBackdrop}>
      ...
      >
        ...
        <h2 id="safety-dialog-title">
          {isPause ? copy.confirmPauseTitle : copy.confirmResumeTitle}
        </h2>
        ...
        <div className={styles.dialogActions}>
          ...
          <button
            className={isPause ? styles.dangerButton : styles.primaryButton}
            onClick={onConfirm}
            type="button"
          >
...
function DashboardLoading({ copy }: { copy: DashboardCopy }) {
  return (
    <main aria-busy="true" aria-label={copy.loading} className={styles.shell}>
      ...
      <div className={styles.skeletonSafety}>
        <i />
        <i />
        <i />
      </div>
