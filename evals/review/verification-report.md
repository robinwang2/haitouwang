...
- 真实 `AppModule` + Nest HTTP listener：批准接口只信任签名访问令牌中的主体，覆盖无令牌 401、过期令牌 401、缺失/字符串/非整数 `exp` 401、错误受众 403、错误签名 401、缺少审批权限 403、跨租户资源 404 和合法审批者 200。
- 可部署鉴权：`AuthService` 验证 HS256 签名，并要求 `exp` 是严格整数且晚于当前 Unix 秒；缺失、字符串和非整数 `exp` 均按 `TOKEN_EXPIRED` 拒绝。服务同时验证 `iat`、`aud`、UUID 主体和权限声明；应用不再包含测试 token registry。测试在应用启动前注入测试签名密钥并独立签发短期令牌。
...
- 回滚验证：HTTP 测试在真实 SQLite 表安装失败触发器，使批准成功审计写入失败；接口返回契约化 500，材料版本、Review 和成功审计均回滚，只在新事务中留下完整的 `material.approval_failed` 事件。拒绝流程另有真实文件型 SQLite 触发器测试：`material.rejected` 写入失败时材料版本与 Review 保持原状、半成品成功审计不存在，并由独立事务持久化 `material.rejection_failed`。另有应用重启测试确认成功批准、Review 和审计均可重新读取。
- HTTP 契约：`review_id` 在运行时强制为必填 UUID；请求拒绝未知字段。Material 成功响应只映射契约声明字段，所有错误使用带 `retryable`、`request_id` 和 `correlation_id` 的 `ErrorEnvelope`，领域门禁原因映射到共享错误码。批准与拒绝的请求体校验失败均生成门禁审计；拒绝端点的 OpenAPI 审计契约和领域 AuditEvent Schema 已同步记录 `material.rejection_failed`。测试以 Draft 2020-12 Schema 验证真实 HTTP Material、Review、AuditEvent 和 ErrorEnvelope。
...
| `npm.cmd run unit`                    | 通过；12 个测试文件，114 passed、1 real-provider smoke skipped；包含 13 条真实 Nest HTTP/SQLite 联调用例 |
